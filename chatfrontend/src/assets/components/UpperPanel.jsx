    ...
    if (isMobile) {
        return (
            <>
                ...
                <div className='w-full min-h-[8%] absolute top-0 h-[8%] bg-950 border-b box-border p-[2.5%] px-[3%] border-600 flex justify-between items-center'>
                    ...
                    <a href="/" className='h-full'>
                        <div className="max-w-full w-max h-full flex items-center justify-start gap-3 tracking-widest select-none box-content">
                            ...
                            <div className="ysab leading-none flex flex-col">
                                <div className="clamp-title text-300 font-black lg:leading-none md:leading-none decoration-0 text-[0.8rem]">
                                    COLLOCUS
                                </div>
                                <div className="clamp-subtitle mt-1 text-500 block text-[6px]">
        ...
        return (
            <div className={`w-full min-h-8 h-8 bg-950 border-b box-content overflow-y-visible p-3 flex border-600 ${props.disabled && "pointer-events-none"}`}>
                <div className='w-[17.5rem] flex h-full border-r border-600 box-content pr-2'>
                    ...
                    <div className='w-[69%] h-full ml-[4%] flex flex-col justify-between'>
                        <div className='text-[14px] max-h-[50%] max-w-full ysab font-bold overflow-hidden leading-4 text-300 whitespace-nowrap text-ellipsis'>
                            {props.disabled && "Alexander"}
                            {user.loggedIn && user.username}
                        </div>
                        <div className='text-[10px] max-h-[50%] max-w-full quicksand font-medium overflow-hidden leading-2 text-400 whitespace-nowrap text-ellipsis'>
                    ...
                    <div className='h-full aspect-square ml-auto' onClick={() => props.setProfileModalOpen(true)}>
                        <div className='min-h-full aspect-square mr-2 cursor-pointer grid place-items-center fill-400 hover:fill-300 duration-150'>
                ...
                <div className='box-border w-[calc(100%-20rem)] max-w-[calc(100%-20rem)] ml-[0.66rem] overflow-x-auto overflow-y-hidden flex gap-2 justify-start h-full pointer-events-auto'>
                    ...
                    <div onClick={() => props.setChannelModalOpen(prev => !prev)} className='h-full aspect-square rounded-full box-border border duration-100 cursor-pointer border-500 hover:border-300 grid place-items-center fill-500 hover:fill-300 text-lg leading-none'>
        ...
        <>
            {
                servers.map(server => {
                    return <>
                        <div className='h-full aspect-square'>
                            <div id={server._id} className='opacity-0 scale-[0.9] rounded-tl-none duration-100 z-[999] absolute text-200 text-sm top-12 quicksand pointer-events-none font-medium py-1 px-2 rounded-lg bg-900 border border-500'>
                                {server.name}
                            </div>
                            <div key={server._id} onClick={() => changeChannels(server._id)} onMouseEnter={(e) => hovered(server._id, "enter")} onMouseLeave={(e) => hovered(server._id, "leave")} className='h-full aspect-square rounded-full box-border border duration-100 cursor-pointer border-500 hover:border-300 grid place-items-center fill-500 hover:fill-300 text-lg leading-none'>
                                <img 
