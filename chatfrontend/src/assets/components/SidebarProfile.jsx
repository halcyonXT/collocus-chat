...
export default function SidebarProfile(props) {
    ...
    if (!props.svg) {return (
        <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} className='w-full py-1 flex cursor-pointer items-center select-none mb-2 justify-start gap-2 duration-150 rounded-full box-content hover:bg-900 h-6'>
            ...
            <span className='grow ml-2 quicksand overflow-hidden whitespace-nowrap text-ellipsis text-600 leading-5 text-base'>
                {props.name}
            </span>
            {
                hovered &&
                <>
                    <div className='min-h-full aspect-square grid place-items-center fill-600 hover:brightness-125 duration-150'>
                    ...
                    <div className='min-h-full aspect-square mr-2 grid place-items-center fill-600 hover:brightness-125 duration-150'>
        ...
        <div onClick={handleCopyLink} className={`w-full py-1 flex cursor-pointer items-center mb-2 justify-start gap-2 duration-150 rounded-full box-content ${clicked ? 'bg-accent fill-white' : 'hover:bg-900'} h-6 fill-accent hover:brightness-125 text-accent`}>
            ...
            <span className={`grow ml-2 quicksand overflow-hidden whitespace-nowrap text-ellipsis ${clicked ? 'text-white' : 'text-accent'} leading-5 text-base`}>
                {
                    clicked
                    ?
                    "Copied channel ID"
