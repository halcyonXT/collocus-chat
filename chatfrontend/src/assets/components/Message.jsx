        ...
        ${["bottom", "none"].includes(props.chain) && 'mb-4'} p-[3.5%] ${["bottom", "middle"].includes(props.chain) && "pt-0 md:pt-0 lg:pt-0"} md:p-[2.5%] md:pb-0 lg:p-[1%] lg:pb-0 pb-0 bg-opacity-60 flex flex-col justify-start items-start rounded-2xl`}>
            {
                ...
                <div className='h-max flex w-full max-w-full items-end gap-4'>
                    ...
                    <div className='h-full flex flex-col justify-evenly'>
                        <div className={`text-600 ${!isMobile ? 'text-[1rem]' : 'text-[15px]'} max-w-[65vw] font-semibold ysab overflow-hidden whitespace-nowrap text-ellipsis`}>
                            {props.name}
                        </div>
                        <div className={`${!isMobile ? "text-[13px]" : "text-[12px]"} text-700 leading-none quicksand`}>
                            {props.timestamp}
                        </div>
                    </div>
                </div>
            }
            <div className={`quicksand ${!isMobile ? "text-[1rem]" : 'text-[1rem]'} text-600 ml-[20px] break-words h-max max-w-[95%] pl-[calc(20px+1rem)] font-medium leading-8 border-l border-700`}>
                {props.content}{props.chain !== "top" && hovered && <span className='text-xs ml-2 text-700 pointer-events-none'>{props.timestamp.slice(-8)}</span>}
            </div>
        </div>
    )
}