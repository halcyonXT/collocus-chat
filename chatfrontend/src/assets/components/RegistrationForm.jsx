    ...
    return (
        <>
            <div className='w-screen h-[100dvh] grid place-items-center bg-900'>
                <div className='w-96 relative scale-110 rounded-lg h-max p-4 max-w-[calc(100vw-3rem)] max-h-[calc(100dvh-2rem)] bg-950 flex flex-col border border-700 shadow-sm shadow-black'>
                    <div className='flex pb-4 border-b border-700 mb-1 justify-between'>
                        <a href="/" className='h-max w-[80%] max-w-[80%]'>
                            <div className="w-[80%] max-w-[80%] h-8 flex items-center justify-start gap-3 tracking-widest select-none box-content">
                                ...
                                <div className="ysab leading-none flex flex-col">
                                    <div className="text-accent font-black lg:leading-none md:leading-none decoration-0 text-[0.8rem]">
                                        COLLOCUS
                                    </div>
                                    <div className="mt-1 text-600 text-[7px]">
                        ...
                        <Link to="/">
                            <div className='aspect-square grid duration-150 cursor-pointer place-items-center fill-600 h-8'>
                    ...
                    <div className='w-full h-max flex flex-col overflow-x-hidden overflow-y-auto pb-2 border-b mb-4 border-700'>
                        ...
                        <div className='relative h-max flex items-center overflow-visible py-1 pt-3'>
                            <Input
                                id="username-input"
                                tether={info.username}
                                callback={(e) => setInfo(prev => ({...prev, username: e.target.value}))}
                                name="Username"
                            />
                        </div>
                        <div className='relative h-max flex items-center overflow-visible py-1 pt-3'>
                            <Input
                                id="email-input"
                                tether={info.email}
                                callback={(e) => setInfo(prev => ({...prev, email: e.target.value}))}
                                name="Email"
                            />
                        </div>
                        <div className='relative h-max flex items-center overflow-visible py-1 pt-3'>
                            <Input
                                id="password-input"
                                tether={info.password}
                                callback={(e) => setInfo(prev => ({...prev, password: e.target.value}))}
                                name="Password"
                                type="password"
                                visibilityTether={passwordVisible}
                            />
                            <div className='absolute right-2 cursor-pointer fill-700 duration-150 hover:fill-300' onClick={() => setPasswordVisible(prev => !prev)}>
                                {
                                    passwordVisible
                                    ?
                        ...
                        <div className='w-full flex h-max justify-between mt-3 overflow-visible'>
                            <div className='text-600 quicksand text-[10px]'>
                                By checking this box you agree to follow our&nbsp;
                                <span className='text-accent cursor-pointer underline' onClick={() => setTermsActivated(prev => !prev)}>Terms and Conditions</span>
                            </div>
                        ...
                        <div onClick={handleFormSubmit} className='py-[0.3rem] mt-3 mb-2 mx-auto font-medium text-950 bg-accent hover:brightness-125 rounded-lg cursor-pointer text-center w-40 ysab tracking-widest text-sm duration-150'>
                            REGISTER
                        </div>
                    </div>
                    <div className='w-full text-center text-xs quicksand text-600'>
                        Already got an account? <Link to="/login"><span className='text-accent underline font-semibold'>Log in.</span></Link>
                    </div>
                    {
                        status.active &&
                        <>
                ...
                </div>
                {
                    termsActivated
                    &&
                    <dialog open className='w-[500px] items-center max-w-[95vw] box-border p-4 rounded-lg h-[600px] max-h-[95vh] border border-700 bg-950 text-600 flex flex-col justify-between'>
                        <div className='h-[92%] max-h-[92%] w-full whitespace-pre-wrap overflow-y-auto overflow-x-hidden text-sm shadow-sm'>
                            <span className='text-accent text-base font-semibold text-center w-full'>Terms and Conditions for Collocus Chat App</span><br/><br/>
                            {
